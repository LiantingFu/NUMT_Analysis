...
parser = argparse.ArgumentParser(description="merge NUMTs from different chrM reference sequence")
parser.add_argument("input_records", nargs="+", help="liftOvered NUMT records")
...
df_info_merge_unmapped.to_csv(f"{work_dir}/{prefix}.unMapped.tsv", sep="\t", index=False)
df_info_merge = df_info_merge.dropna()

# output bed file for igv
df_info_merge_igv_bed_columns = df_info_merge[["nu_chrom", "nu_start", "nu_end", "numt_id", "mt_start_adjust", "mt_end_adjust", "score_adjust", "strand_adjust"]]
df_info_merge_igv_bed = df_info_merge_igv_bed_columns.apply(
    lambda row: f"{row['nu_chrom']}\t{row['nu_start']}\t{row['nu_end']}\t{row['numt_id']}.chrM:{row['mt_start_adjust']}-{row['mt_end_adjust']}\t{row['score_adjust']}\t{row['strand_adjust']}", axis=1
)
df_info_merge_igv_bed.to_csv(f"{work_dir}/{prefix}.IGV.bed", index=False, header=False)

# multiple numt results intersect: r>=0.8 or R>=0.8
df_info_merge_bed_format = df_info_merge[["nu_chrom", "nu_start", "nu_end", "numt_id", "score_adjust", "strand_adjust"]].to_csv(sep="\t", index=False, header=False)
...
df_info_merge.set_index("numt_id", inplace=True)
